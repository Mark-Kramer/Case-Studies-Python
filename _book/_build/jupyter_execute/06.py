...
### Goals
Our goal is to better understand whether an evoked response appears in the data. We first make a visual inspection of the data, using techniques we developed to <a href="02" target="_blank">study evoked responses</a>. However, our main goal is to understand the fundamental procedures of filtering neural field data, and we examine filtering methods applied to these example EEG data. We start by developing an intuitive approach to filtering and then implement and apply more sophisticated methods. We explain procedures to visualize filter properties and the resulting impact on the input signal.

### Tools
In this notebook, we rely on the Fourier transform (and associated measures) to develop a basic understanding of filters. If you are not confident using the Fourier transform, we strongly recommend reviewing notebooks <a href="03" target="_blank">3</a> and <a href="04" target="_blank">4</a>. This case study reinforces concepts in those notebooks and provides another opportunity to compute and to examine spectra, and to examine the relationships between the time and frequency domain representations of a time series. Upon completing this notebook, you should be familiar with basic filtering principles and methods to visualize the impact of filters, and equipped for further study and development of filtering procedures.
...
Initial visual inspection of the single-trial data suggests that a 60 Hz rhythm dominates each individual trial. To further characterize this observation, let’s compute the spectrum of a single trial of EEG data <sup><abbr title="We could instead write the sample spectrum because this equation uses the observed data to estimate the theoretical spectrum that we would see if we kept repeating this experiment, but this distinction is not essential to the discussion here.">note</abbr></sup>. We do so here for the first trial, and apply a Hanning taper before computing the spectrum (see <a href="04" target="_blank">notebook 4</a>):
...
**A.** The total duration of a trial is 1 s. Therefore the frequency resolution is 1/(1 s) = 1 Hz. If this answer makes no sense, we recommend reviewing the case study in <a href="03" target="_blank">notebook 3</a>.
...
We take the Fourier transform of the filter, with zero padding (see <a href="04" target="_blank">notebook 4</a>) to match the size of the EEG data, and compute the product of the Fourier transform of the filter and its complex conjugate. We then define the frequency axis, and plot the sorted frequency axis to avoid any unwanted lines.
...
In the preceding code, we applied the FIR filter by computing the convolution. To conclude this section, we introduce a function from the <a href="https://docs.scipy.org/doc/scipy/reference/signal.html" target="_blank">SciPy Signal module</a> to apply the FIR filter:
...
In general, in neuroscience applications, it’s often useful to remove phase distortion through zero-phase filtering. The <a href="https://docs.scipy.org/doc/scipy/reference/signal.html" target="_blank">SciPy Signal module</a> provides a simple function to perform zero-phase filtering,
...
The design and application of filters is an enormous and rich field of study. The goal of this notebook is not a thorough discussion of filtering. Instead, we introduced only a handful of filtering concepts that motivate a basic understanding of filtering. These concepts extend directly from ideas developed to compute the spectrum in notebooks <a href="03" target="_blank">3</a> and <a href="04" target="_blank">4</a>. Through tools such as the Fourier transform and convolution, we are able to visualize and apply filters in the frequency and time domains. These same tools apply and provide context for alternative approaches to filtering. For further details in the design and application of filters see [<a href="https://doi.org/10.1017/CBO9780511622762" target="_blank">Percival & Walden, 1998</a>, <a href="https://buprimo.hosted.exlibrisgroup.com/permalink/f/1du03mk/ALMA_BOSU121668583370001161" target="_blank">Priestley, 1981</a>]. We apply filters in <a href="07" target="_blank">notebook 7</a> to assess cross-frequency coupling in neural field data.
